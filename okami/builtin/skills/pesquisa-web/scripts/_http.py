"""Tiny stdlib HTTP helper used by the pesquisa-web scripts (search_arxiv.py, fetch_wikipedia.py).

Provides polite retry + JSON convenience + User-Agent enforcement. Ported from Hermes Agent's
osint-investigation skill (MIT).
"""
...
DEFAULT_UA = (
    "okami-pesquisa-web/1.0 "
    "(+https://github.com/OkamiAgent/okami-agent; "
    "set OKAMI_RESEARCH_UA to identify yourself per "
    "Wikimedia / arXiv fair-use guidance)"
)
...
def get(
    ...
    *,
    ...
    h = {"User-Agent": user_agent or os.environ.get("OKAMI_RESEARCH_UA", DEFAULT_UA)}
    if headers:
        h.update(headers)
