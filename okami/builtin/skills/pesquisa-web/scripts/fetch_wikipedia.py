...
Two free APIs:
  ...
  - Wikidata Action API for structured facts (birth, employer, awards, etc.)

Both are anonymous-access. Useful for resolving who-is-this-entity questions
and building an evidence chain that other sources can join against — the
same cross-referencing pattern Hermes' osint-investigation skill uses, kept
here in its general-research form (no financial/legal-registry scripts).

Usage:
    python3 fetch_wikipedia.py --query "Ada Lovelace" --out /tmp/ada.csv
    python3 fetch_wikipedia.py --query "Acme Corp" --limit 3 --no-wikidata --out /tmp/acme.csv
"""
...
def main() -> int:
    ...
    p.add_argument(
        "--no-wikidata",
        action="store_true",
        help="Skip the Wikidata enrichment (faster, less detail)",
    )
