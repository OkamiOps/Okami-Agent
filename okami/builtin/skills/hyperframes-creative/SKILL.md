...
description: Direção criativa (não-animação) pra vídeos HyperFrames — design spec (frame.md/design.md), paletas, tipografia, narração, planejamento de beat, visual audio-reativo, padrões de composição, decisões de marca/estilo. Pra padrões atômicos de motion e blueprints de cena, use `hyperframes-animation`.
triggers: [direção criativa video, paleta de video, narração, design spec video, beat planning, house style, composição de video]
intent_examples:
  - "escolhe uma direção visual pra esse vídeo de produto"
  - "que paleta combina com esse vídeo dark tech"
  - "planeja o roteiro/narração desse explicador"
metadata:
  source: https://github.com/heygen-com/hyperframes (skills/hyperframes-creative)
  hermes:
    tags: [video, animation, design, creative-direction, hyperframes]
    related_skills: [hyperframes, hyperframes-animation]
    category: creative
---
