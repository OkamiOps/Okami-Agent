...
- **Tension can live inside a single font.** A font that looks familiar but is unexpectedly strange creates tension with the viewer's expectations, not with another font.
...
Save this script to `/tmp/fontquery.py`, download `https://fonts.google.com/metadata/fonts` to `/tmp/gfonts.json` with an HTTP client, then run `python3 /tmp/fontquery.py /tmp/gfonts.json`:
