...
Usage (wire into cron via the Okami scheduler, --no-agent style, or run ad-hoc):

    python3 watch_rss.py --name hn --url https://news.ycombinator.com/rss --max 5
...
def main() -> int:
    ...
    try:
        req = urllib.request.Request(args.url, headers={"User-Agent": "Okami-Watcher/1.0"})
