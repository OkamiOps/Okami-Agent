...
description: "Playbook de marketing/automacao LinkedIn — pipeline de conteudo, geracao de leads, sequencia de outreach, ads, analytics. SEM integracao embutida: precisa de MCP/API LinkedIn autorizada (nao vem com credencial nenhuma). Fonte: claude-office-skills/skills."
triggers: [linkedin, outreach linkedin, lead generation linkedin, post linkedin, automacao linkedin, ads linkedin, sequencia de conexao]
intent_examples:
  - "monta um pipeline de conteudo pro LinkedIn da empresa"
  - "escreve uma sequencia de outreach pra leads de CTO"
  - "quero um plano de post no formato storytelling"
  - "como estruturo uma campanha de ads no LinkedIn"
---

## Aviso de autenticacao (leia antes de usar)

Esta skill e um **playbook de conteudo/estrategia** — templates de post, sequencias de
outreach, estrutura de lead scoring, calendario. Ela **nao inclui nenhuma credencial,
token ou integracao pronta** com o LinkedIn.

O frontmatter original desta skill (upstream) referencia um servidor MCP fictício
chamado `social-mcp` com ferramentas `linkedin_post`/`linkedin_message`/etc. Esse
servidor **nao existe no Okami**. Antes de tentar publicar, mandar mensagem ou puxar
analytics de verdade:

1. Confirme com o usuario que existe uma integracao LinkedIn autorizada disponivel
   (API oficial do LinkedIn via OAuth, ou um MCP de terceiros que ele já conectou).
2. **Nunca** tente automatizar o LinkedIn via scraping/RPA de browser sem autorizacao
   explicita — isso viola os Termos de Servico do LinkedIn e pode banir a conta do
   usuario.
3. Sem integracao disponivel, use esta skill só para **gerar o conteudo** (posts,
   mensagens, estrategia) e deixe a publicacao/envio manual a cargo do usuario.

Nada abaixo desta secao pressupoe que a integracao existe — os blocos YAML são
templates de estrutura, não chamadas de API reais.
