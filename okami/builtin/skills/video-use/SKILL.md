---
name: video-use
description: "Editar video por conversa — transcrever (ElevenLabs Scribe), cortar, gradear cor, gerar overlays, legendar. Sem presets fixos; regras de correcao de producao sao obrigatorias, resto e liberdade criativa. Requer ffmpeg + deps Python + chave ElevenLabs (nao inclusa). Fonte: browser-use/video-use."
triggers: [editar video, cortar video, transcrever video, legenda, video-use, montagem, corte de video, gradear cor video]
intent_examples:
  - "edita essas takes numa vinheta de lancamento"
  - "transcreve esses videos e monta o corte"
  - "faz a legenda com esse estilo bold-overlay"
  - "gradeia essa cena com um look cinematico quente"
metadata:
  requires_deps: [ffmpeg, ffprobe, "python: requests, librosa, matplotlib, pillow, numpy"]
  requires_credential: ELEVENLABS_API_KEY
---
...
Helpers (`scripts/transcribe.py`, `scripts/render.py`, etc.) live alongside this SKILL.md. Resolve their paths relative to the directory containing this file — the skill is typically symlinked at `~/.claude/skills/video-use/` or `~/.codex/skills/video-use/`.
