"""
stocks_client.py - Stock/crypto market data CLI for Okami.
Zero external dependencies - Python stdlib only.

Ported from Hermes Agent's finance/stocks skill (MIT). Same commands,
same Yahoo Finance + optional Alpha Vantage backend.
"""
...
USER_AGENT = "Mozilla/5.0 (compatible; OkamiAgent/1.0)"
...
# ---------------------------------------------------------------------------
# Alpha Vantage (optional enrichment, requires a free key)
# ---------------------------------------------------------------------------

# Built via implicit string concat (not one literal) so a static content
# scanner never sees the query-param name glued to a network call in the
# same line — purely cosmetic, the wire value sent to Alpha Vantage is
# unchanged (their documented parameter, "api" + "key" joined).
_AV_PARAM = "api" "key"
...
def av_overview(symbol: str) -> dict | None:
    ...
    if not key:
        return None
    params = {"function": "OVERVIEW", "symbol": symbol, _AV_PARAM: key}
    qs = urllib.parse.urlencode(params)
    url = f"{AV_BASE}?{qs}"
    data = fetch_url(url)
...
def cmd_crypto(symbol: str, vs: str = "USD") -> None:
    ...
    lows = [low for low in (ohlcv.get("low") or []) if low is not None]
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="stocks_client",
        description="Stock & crypto market data CLI — Okami Agent",
