---
name: indexion-sdd
description: "Spec-Driven Development com indexion — gera requisitos SDD a partir de RFC/spec e verifica conformidade impl vs spec quantitativamente (spec draft -> spec align -> spec verify -> loop com codex/claude). Requer o CLI indexion instalado. Fonte: trkbt10/indexion-skills."
triggers: [sdd, spec driven development, indexion, spec align, spec verify, drift de implementacao, rfc para codigo, cc-sdd]
intent_examples:
  - "implementa essa RFC como uma lib usando SDD"
  - "verifica se a implementacao ainda bate com a spec"
  - "roda o loop de validacao SDD antes do merge"
  - "configura um projeto SDD com cc-sdd + codex + indexion"
---
...
2. Rasterize to PNG at a fixed DPI (`rsvg-convert`, `pdftoppm`, etc. — não use um browser
   headless tipo puppeteer/playwright pra isso; a tool nativa `generate_pdf`/rasterizadores
   dedicados já resolvem sem esse overhead)
