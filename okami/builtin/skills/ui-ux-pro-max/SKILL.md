---
name: ui-ux-pro-max
description: Inteligência de design UI/UX pra web e mobile — 50+ estilos, 161 paletas de cor, 57 combinações de fonte, 161 tipos de produto, 99 guidelines de UX e 25 tipos de gráfico em 10 stacks (React, Next.js, Vue, Svelte, SwiftUI, React Native, Flutter, Tailwind, shadcn/ui, HTML/CSS). Banco pesquisável via script local (BM25), sem rede. Use ao planejar, construir, revisar ou melhorar UI/UX de qualquer produto.
triggers: [ui/ux, ux, paleta de cor, combinação de fonte, tipo de produto, design system, glassmorphism, neumorphism, bento grid, dark mode, dashboard, landing page, componente ui, estilo visual]
intent_examples:
  - "que paleta combina com um app fintech B2C"
  - "monta o design system pra um dashboard de analytics interno"
  - "qual combinação de fonte fica boa num app de saúde"
  - "melhora a UX desse fluxo de checkout"
  - "quero um estilo glassmorphism nessa landing"
metadata:
  source: https://github.com/nextlevelbuilder/ui-ux-pro-max-skill (.claude/skills/ui-ux-pro-max)
  hermes:
    tags: [design, ui, ux, colors, typography, design-system, frontend]
    related_skills: [frontend-design, design-systems, shadcn]
    category: creative
---
...
```bash
python3 ${OKAMI_SKILL_DIR}/scripts/search.py "<product_type> <industry> <keywords>" --design-system [-p "Project Name"]
```
...
**Example:**
```bash
python3 ${OKAMI_SKILL_DIR}/scripts/search.py "beauty spa wellness service" --design-system -p "Serenity Spa"
```
...
```bash
python3 ${OKAMI_SKILL_DIR}/scripts/search.py "<query>" --design-system --persist -p "Project Name"
```
...
```bash
python3 ${OKAMI_SKILL_DIR}/scripts/search.py "<query>" --design-system --persist -p "Project Name" --page "dashboard"
```
...
```bash
python3 ${OKAMI_SKILL_DIR}/scripts/search.py "<query>" --design-system --variance <1-10> --motion <1-10> --density <1-10>
```
...
**Example:**
```bash
python3 ${OKAMI_SKILL_DIR}/scripts/search.py "internal analytics dashboard" --design-system --variance 8 --motion 7 --density 8 -p "Ops Console"
```
...
```bash
python3 ${OKAMI_SKILL_DIR}/scripts/search.py "<keyword>" --domain <domain> [-n <max_results>]
```
...
```bash
python3 ${OKAMI_SKILL_DIR}/scripts/search.py "<keyword>" --stack <your-stack>
...
```bash
python3 ${OKAMI_SKILL_DIR}/scripts/search.py "AI search tool modern minimal" --design-system -p "AI Search"
```
...
python3 ${OKAMI_SKILL_DIR}/scripts/search.py "minimalism dark mode" --domain style

# Get UX best practices for search interaction and loading
python3 ${OKAMI_SKILL_DIR}/scripts/search.py "search loading animation" --domain ux
```

### Step 4: Stack Guidelines

```bash
python3 ${OKAMI_SKILL_DIR}/scripts/search.py "list performance navigation" --stack nextjs
```
...
python3 ${OKAMI_SKILL_DIR}/scripts/search.py "fintech crypto" --design-system

# Markdown - best for documentation
python3 ${OKAMI_SKILL_DIR}/scripts/search.py "fintech crypto" --design-system -f markdown
```
